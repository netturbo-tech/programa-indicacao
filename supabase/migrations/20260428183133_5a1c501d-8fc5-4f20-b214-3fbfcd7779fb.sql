...
CREATE OR REPLACE FUNCTION private.indicacao_update_allowed(
  _id uuid,
...
BEGIN
  ...
  IF private.has_role(auth.uid(), 'aprovador') THEN
...
GRANT EXECUTE ON FUNCTION private.indicacao_update_allowed(uuid, public.status_indicacao, text, text, text, text, public.produto_tipo, text, public.setor_tipo, text, public.contrato_tipo, text, uuid, text, text, boolean) TO authenticated;

DROP POLICY IF EXISTS "Users view own indicacoes, approvers and admins view all" ON public.indicacoes;
DROP POLICY IF EXISTS "Users update own indicacoes, approvers only status, admins all" ON public.indicacoes;
DROP POLICY IF EXISTS "Users delete own indicacoes, admins delete all" ON public.indicacoes;
...
USING (
  ...
  OR private.has_role(auth.uid(), 'aprovador')
);
...
WITH CHECK (
  private.indicacao_update_allowed(
    id, status, lead_nome, empresa, telefone, email_lead, produto, email_indicador,
    setor, funcao, contrato, observacao, criado_por_id, criado_por_nome,
    modificado_por_nome, recompensa_paga
  )
);
...
REVOKE EXECUTE ON FUNCTION private.indicacao_update_allowed(uuid, public.status_indicacao, text, text, text, text, public.produto_tipo, text, public.setor_tipo, text, public.contrato_tipo, text, uuid, text, text, boolean) FROM anon;